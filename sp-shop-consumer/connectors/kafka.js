  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        const x = await axios.post('https://analytics-wc.vercel.app/api/v1/analysis/types',parsedMessage)
        if(messageType===messages.TICKET_RESERVED){
        const y = await axios.post('https://analytics-wc.vercel.app/api/v1/analysis/teamsAnalysis',parsedMessage)
        }
